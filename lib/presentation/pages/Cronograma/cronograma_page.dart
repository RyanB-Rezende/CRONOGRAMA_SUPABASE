...
class _CronogramaPageState extends State<CronogramaPage> {
  ...
  Future<void> _carregarAulas() async {
    try {
      ...
      for (var aula in response) {
        try {
          final dateStr = aula['data'] as String;
          final date = dateStr.contains('T')
              ? DateTime.parse(dateStr).toLocal()
              : DateTime.parse(dateStr);
          final normalizedDate = DateTime(date.year, date.month, date.day);
          ...
          final aulaObj = Aula(
            ...
            status: aula['status'] as String? ?? 'Agendada',
  ...
  Future<Map<String, dynamic>> _getAulaDetails(int idAula) async {
    try {
      final response = await Supabase.instance.client.from('Aulas').select('''
          ...
          Turma(turma, Instrutores(nome_instrutor))
        ''').eq('idAula', idAula).single();

      return {
        'nome_uc':
            response['Unidades_Curriculares']['nome_uc'] ?? 'Não encontrado',
        'turma': response['Turma']['turma'] ?? 'Não encontrada',
        'nome_instrutor': response['Turma']['Instrutores']['nome_instrutor'] ??
            'Não encontrado',
